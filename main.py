...
async def parse_robota_ua_playwright() -> list:
    ...
    try:
        ...
        cards = soup.select("alliance-employer-list-item") or soup.select("article") or soup.select("div.vacancy")
        log.info(f"Robota.ua: знайдено {len(cards)} карток")

        for card in cards[:30]:
            a = card.select_one("a[href*='/vacancy/']") or card.select_one("a")
            if not a:
                continue
            href = a.get("href", "")
            if not href:
                continue
            uid = "robota_" + href.strip("/").split("/")[-1]
            title_el = card.select_one("h2, h3, .title")
            title = title_el.get_text(strip=True) if title_el else a.get_text(strip=True)
            if not title or len(title) < 3:
                continue
            company_el = card.select_one(".company-name, .company, a.company")
            company = company_el.get_text(strip=True) if company_el else "—"
            ...
            if not is_allowed_location(location):
                continue
            url = href if href.startswith("http") else "https://robota.ua" + href
...
async def parse_djinni_playwright() -> list:
    ...
    try:
        async with async_playwright() as p:
            ...
            await page.wait_for_selector("a.job-link", timeout=15000)
        ...
        log.info(f"Djinni: знайдено {len(cards)} карток")

        for card in cards[:30]:
            href = card.get("href", "")
            if not href:
                continue
            uid = "djinni_" + href.strip("/").split("/")[-1]
            ...
            if not title:
                continue
            company_el = card.select_one(".job-item__company, .company-name")
            company = company_el.get_text(strip=True) if company_el else "—"
            salary_el = card.select_one(".public-salary-item, .job-item__salary")
            salary = salary_el.get_text(strip=True) if salary_el else ""
            loc_el = card.select_one(".location-text, .job-item__location")
            location = loc_el.get_text(strip=True) if loc_el else "Віддалено"
            exp_el = card.select_one(".years-exp, .job-item__experience")
            experience = exp_el.get_text(strip=True) if exp_el else ""
            desc_el = card.select_one(".job-item__description, .truncated")
            description = desc_el.get_text(strip=True)[:280] if desc_el else ""
            if not is_allowed_location(location):
                continue
            url = href if href.startswith("http") else "https://djinni.co" + href
